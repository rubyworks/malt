...
We can also get a hold of the Sass document via the Malt.file function.
  ...
  scss = Malt.file('tmp/test.sass')

  scss.class.assert == Malt::Formats::Sass

We can convert the Sass document to a CSS document via the #to_css method.

  css = sass.to_css

We can see that the output is an instance of Malt::Formats::HTML.

  css.class.assert == Malt::Formats::CSS
